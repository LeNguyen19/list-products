...
export default function Index() {
  ...
  function dropdownItemEffectClick(id) {
    setSelectedEffect(listEffect[id - 1].nameEffect);
    setIsActive(false);
    document.getElementById("border__dropdown__city").style.border =
      "1px solid #D9D9D9";
  }
  ...
  function SeenDetail(id) {
    if (listEvent[id - 1].form === "Tặng hàng") {
      navigate("/Admin/Event/DetailEventBonus");
    }
    if (listEvent[id - 1].form === "Khuyến mãi theo sản phẩm") {
      navigate("/Admin/Event/DetaiEventDiscount");
    }
    if (listEvent[id - 1].form === "Khuyến mãi theo hoá đơn") {
      navigate("/Admin/Event/DetaiEventInvoiceDiscount");
    }
  }

  // TÌM KIẾM CHƯƠNG TRÌNH KHUYẾN MÃI --------------------------------------------------------
  const [contentFindEvent, setContentFindEvent] = useState();

  function FindEvent() {
    console.log("Hiệu lực: ", selectedEffect);
    console.log("Tên chương trình: ", contentFindEvent);
  }

  // XOÁ CHƯƠNG TRÌNH KHUYẾN MÃI -------------------------------------------------------------
  function DeletedEvent(id) {
    alert("Mã sự kiện cần xoá: " + id);
  }
            ...
            <div className="row-jcsb">
              <div className="group__effect row-jcsb">
                ...
                <div className="input__effect">
                  <div className="dropdown">
                    ...
                    >
                      <input
                        className="value__cbb__find__event"
                        type="text"
              ...
              <div className="group__input row-jcsb">
                <div className="input__find">
                  <input
                    className="input value__input__find__event"
                    placeholder="Nhập tên chương trình cần tìm..."
                    onChange={(e) => setContentFindEvent(e.target.value)}
                  />
                </div>
          ...
          <div className="box">
            <div className="row-jcsb">
              ...
              <Link to="/Admin/AddEvent">
                <button className="button">Thêm chương trình khuyến mãi</button>
              </Link>
            </div>
            <div class="table">
              <table>
                <tr>
                  ...
                  <th className="seen__event">Xoá</th>
                </tr>
                {listEvent.map((e) => (
                  <tr>
                    ...
                    <td className="deleted__event">
                      <i
                        class="fa fa-trash"
                        onClick={() => DeletedEvent(e.id)}
                      ></i>
                    </td>
                  </tr>
                ))}
