import React, { useState } from "react";
import "./style.css";
import Menu from "../../components/MenuWarehouse/index";
import Header from "../../components/Header/index";
import iconInvoice from "../../assets/images/icon-invoice.svg";
import { Link } from "react-router-dom";

export default function Index() {
  // Data --------------------------------------------------------------
  const [infoProductReturn, setInfoProductReturn] = useState({
    nature: "Chờ duyệt",
    listProductReturn: [
      {
        id: 1,
        nameProduct: "Lancome La Vie Est Belle",
        trademark: "Lancome",
        nature: "Hồng đất",
        dateImport: "01/11/2022",
        dateEnd: "01/11/2024",
        price: "210.000đ",
        amount: 10,
        total: "210000đ",
      },
      {
        id: 2,
        nameProduct: "Lancome La Vie Est Belle",
        trademark: "Lancome",
        nature: "Cam cháy",
        dateImport: "01/11/2022",
        dateEnd: "01/11/2024",
        price: "210.000đ",
        amount: 10,
        total: "210000đ",
      },
    ],
  });

  function SendReturnInformation() {
    alert(
      "Nhấn vô rồi gửi mail cho nhà cung cấp danh sách sản phẩm cần trả nè"
    );
  }
  return (
    <div className="container">
      <div className="background">
        <Menu />
      </div>
      <div className="content">
        <Header />
        <div className="wrapper__content">
          <div className="title__content row">
            <span>
              <img src={iconInvoice} alt="" />
            </span>
            <p>Chi tiết kế hoạch trả hàng</p>
          </div>
          <div className="box">
            <p>
              Danh sách sản phẩm <hr />
            </p>

            <br />
            <div class="table">
              <table>
                <tr>
                  <th className="name">Tên sản phẩm</th>
                  <th className="trademark">Thương hiệu</th>
                  <th className="nature__detailProduct">Tính chất</th>
                  <th className="price__detailProduct">Ngày nhập</th>
                  <th className="price__detailProduct">Hạn sử dụng</th>
                  <th className="price__detailProduct">Giá nhập</th>
                  <th className="amout__detailProduct">Số lượng</th>
                  <th className="price__detailProduct">Tổng tiền</th>
                </tr>
                {infoProductReturn.listProductReturn.map((e) => (
                  <tr>
                    <td className="name">{e.nameProduct}</td>
                    <td className="trademark">{e.trademark}</td>
                    <td className="nature__detailProduct">{e.nature}</td>
                    <td className="price__detailProduct">{e.dateImport}</td>
                    <td className="price__detailProduct">{e.dateEnd}</td>
                    <td className="price__detailProduct">{e.price}</td>
                    <td className="amout__detailProduct">{e.total}</td>
                    <td className="price__detailProduct">{e.total}</td>
                  </tr>
                ))}
              </table>
              <hr />
            </div>
            <br />
            <div className="group__button__importGoods">
              <button
                className="button"
                style={
                  infoProductReturn.nature === "Đã duyệt"
                    ? { display: "block" }
                    : { display: "none" }
                }
                onClick={() => SendReturnInformation()}
              >
                Gửi thông tin trả hàng
              </button>
              <button
                className="btn__grey"
                style={
                  infoProductReturn.nature === "Chờ duyệt"
                    ? { display: "block" }
                    : { display: "none" }
                }
              >
                Gửi thông tin trả hàng
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
